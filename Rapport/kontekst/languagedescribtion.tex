...
If an event listener is listening to the event, it can detect when the event is triggered and make sure the event handler is executed. When the behaviour in the event has been run, the tank will continue running the Repeat block.  
...
The Repeat block will iterate through the users code, unless an event has occurred which then have first priority. The Repeat block is the general behaviour and logic of the tank, and will often contain the main movements of the tank. Just as the Setup block, the Repeat block ccan also only be declared once, and have to be in the code.
...
The conditional block is the langauge's conditional statements which will execute the user defiend code in it's body depending on a boolean expression. Conditional blocks helps making the tanks logic for it's behaviour. 
...
\end{table}
\todo[color=red!40]{Please fill me!}
\begin{defi}
Hello? It's me.
\end{defi}

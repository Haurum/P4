...
\textbf{While loop} is needed to make the tanks logic, since the Repeat block is a while loop. The Repeat block will be described in the following chapter.

\textbf{Reserved calls} are methods used to make the tanks behaviour, this is needed to e.g. make the robot shoot, scan for other robots and move around the battlefield. 
...
\textbf{Events} are a big part of Robocode, it gives the robot the option to act on the environment and actions around it. Events are placed in should have because it is not necessary to code a moving tank, but to make the tank more intelligent.
...
\textbf{Cos, Sin \& Tan} could be useful when programming tanks in Robocode, since when the tanks behaviour is made, the user is often working with degrees in the Reserved calls, so if the user wants to make his/her own functions, this could become necessary.
